...
class PageState {
    static initialState() {
        ...
        s.cx = -0.5;
        s.cy = 0;
        // s.perPixel = 3 / window.innerHeight; <- 縦長画面に対応して短辺側を基準とするよう変更(下4行)
        let shortSide = (window.innerHeight < window.innerWidth)
            ? window.innerHeight
            : window.innerWidth;
        s.perPixel = 3 / shortSide;
        s.maxIterations = 500;
        return s;
    }
...
// タイルの横と縦の分割数、ワーカーの個数(使えるCPUのスレッド数)を設定
